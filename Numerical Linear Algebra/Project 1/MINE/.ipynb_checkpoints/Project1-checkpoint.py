...
def Matrix_KKT(G, C, n, m, lamb, s):
    ...
    return Mat, S, Lambdas #returns the KKT matrix, diagonal matrix of slack variables (S), the diagonal matrix of Lagrange multipliers (Lambdas)


def F(x, G, g):
    return 0.5 * np.transpose(x).dot(G).dot(x) + np.transpose(g).dot(x) #returns the value of the objective function.
...
def F_z(x,lamb,s, G, g, C, d):
    ...
    eq3 = s * lamb
    return np.concatenate((eq1, eq2, eq3))  #returns a vector of equations representing KKT conditions
...
def Func_C3(n, maxIter=100, epsilon=10e-16, Print_Time = "Yes", Print_Results = "Yes"):
    ...
    for i in range(maxIter):
        ...
        alpha = Newton_step(lamb, delta[n:n+m], s, delta[n+m:])  #step-size correction function

        mu = s.dot(lamb) / m
        Mu = ((s + alpha * delta[n+m:]).dot(lamb + alpha * delta[n:(n + m)])) / m
        sigma = (Mu/mu)**3

        b[(n + m):] = b[(n + m):] - np.diag(delta[(n + m):]*delta[n:(n + m)]).dot(e) + sigma * mu * e  #correct the sub-step

        delta = np.linalg.solve(Mat, b)

        alpha = Newton_step(lamb, delta[n:(n + m)], s, delta[(n + m):])  #step-size correction with delta
 
        z = z + (alpha * delta) * 0.95  #updating the substep
...
def Func_C4_strategy_1(n, maxIter=100, epsilon=10e-16, Print_Time = "Yes", Print_Results = "No"):
    ...
    for i in range(maxIter):
        ...
        alpha = Newton_step(lamb, delta[n:(n + m)], s, delta[(n + m):])   #step-size correction substep

        z = z + (alpha * delta) * 0.95  #updating substep
...
#comparing the results of the function/program (C3, C4_LDL, C4_Cholesky) for different values of n (5, 50, 100):

#n=5
print("C3")
Func_C3(n=5, Print_Time="Yes", Print_Results="Yes")
print("\n")

print("C4_LDL")
Func_C4_strategy_1(n=5, maxIter=100, epsilon=10e-16, Print_Time="Yes", Print_Results="Yes")
print("\n")

print("C4_Cholesky")
Func_C4_strategy_2(n=5, maxIter=100, epsilon=10e-16, Print_Time="Yes", Print_Results="Yes")
...
print("For matrices and vectors from optpr1, the obtained results are these:")
print("\n")
...
print("For matrices and vectors from optpr2, the obtained results are these:")
print("\n")
