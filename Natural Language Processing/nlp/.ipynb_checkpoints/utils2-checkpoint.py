...
_spacy_cache = {} #adding this to use later to speed up running time
...
def extract_basic_features(df):
    """Apply preprocessing and extract basic features like Jaccard similarity."""
...
def evaluate_model(model, X, y_true):
    """Return AUC, precision, recall scores."""
...
def load_and_split_data(path="./quora_data.csv"):
    """Load dataset and split into train/val/test as specified."""
    df = pd.read_csv(path)
...
def tfidf_cosine_similarity(q1, q2, vectorizer=None):
    if not vectorizer:
...
def extract_improved_features(df, tfidf_vectorizer=None):
    ...
    df['tfidf_cosine'] = df.apply(
        lambda row: tfidf_cosine_similarity(row['question1'], row['question2'], vectorizer=tfidf_vectorizer),
        axis=1
    )
    ...
    return df[['jaccard', 'len_diff', 'tfidf_cosine', 'levenshtein', 'shared_bigrams', 'avg_word_len_diff', 'spacy_cosine']], tfidf_vectorizer
